...
abstract class AbstractPost extends \Magento\Framework\View\Element\Template
{
    ...
    public function getCategoryName($postId)
    {
        $post = $this->getPost();
        if(is_null($post)){
            $post = $this->_postFactory->create()->getCollection()->addFieldToFilter('post_id', $postId)->getFirstItem();
        }
        $categoryId = $post->getResource()->lookupCategoryIds($postId);
        $category =  $this->category->load(end($categoryId));
        return $category->getDataByKey('title');
    }
